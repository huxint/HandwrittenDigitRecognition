...
class DigitRecognitionApp:
    ...
    def __init__(self, root):
        ...
        self.last_x = None
        self.last_y = None
        self.draw_count = 0  # Counter for auto-prediction optimization
    ...
    def setup_gui(self):
        ...
        # Additional bindings for better drawing experience
        self.canvas.bind('<Leave>', self.stop_drawing)  # Stop drawing when mouse leaves canvas
        self.canvas.bind('<Enter>', self.on_canvas_enter)  # Reset when mouse enters canvas
    ...
    def draw_on_canvas(self, event):
        ...
        if self.drawing:
            ...
            # Also draw a circle at current position for better coverage
            ...
            # Auto-predict if model is loaded (reduce frequency for better performance)
            self.draw_count += 1
            if self.model is not None and self.draw_count % 5 == 0:  # Predict every 5th draw event
                self.root.after(50, self.predict_digit)  # Shorter delay for responsiveness
    ...
    def on_canvas_enter(self, event):
        ...
        # Reset drawing state when mouse enters canvas
        if not self.drawing:
            self.last_x = None
            self.last_y = None
